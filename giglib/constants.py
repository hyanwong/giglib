...
class Const:
    NODE_IS_SAMPLE = np.dtype("uint32").type(tskit.NODE_IS_SAMPLE)
    NODE_IS_RE = np.dtype("uint32").type(1 << 19)  # mainly for testing
...
class ValidFlags(IntFlag):
    ...
    IEDGES_INTEGERS = np.dtype("uint32").type(1)  # TODO: replace with auto() for py3.12

    #: If set, intervals are guaranteed finite and have parent span == child span
    IEDGES_INTERVALS = np.dtype("uint32").type(1 << 1)

    #: IF set, each genomic position in a child's chromosome is guaranteed to have
    #: only one interval above
    IEDGES_FOR_CHILD_NONOVERLAPPING = np.dtype("uint32").type(1 << 2)
    ...
    IEDGES_PARENT_OLDER_THAN_CHILD = np.dtype("uint32").type(1 << 3)
    ...
    IEDGES_SAME_PARENT_CHILD_FOR_EDGE = np.dtype("uint32").type(1 << 4)
    ...
    IEDGES_PRIMARY_ORDER_CHILD_TIME_DESC = np.dtype("uint32").type(1 << 9)
    ...
    IEDGES_SECONDARY_ORDER_CHILD_ID_ASC = np.dtype("uint32").type(1 << 10)
